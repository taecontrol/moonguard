        ...
        @foreach ($exceptions as $exception)
            <li class="relative">
                <button type="button" class="w-full p-6 text-left"
                    :class="selected === {{ $loop->index }} ? '!bg-primary-900 !hover:bg-primary-800' :
                        'bg-white hover:bg-primary-50'"
                    @click="selected !== {{ $loop->index }} ? selected = {{ $loop->index }} : selected = null">
                    <div class="truncate">
                        <div class="flex justify-between text-sm"
                            :class="selected === {{ $loop->index }} ? 'text-gray-400' : 'text-gray-500'">
                            <p class="truncate font-medium">{{ $exception->type }}</p>
                            <p>{{ $exception->thrown_at->toCookieString() }}</p>
                        </div>
                        <div class="mt-2 flex">
                            <p class="text-xl font-bold"
                                :class="selected === {{ $loop->index }} ? 'text-white' : 'text-gray-900'">
                                {{ $exception->message }}
                            </p>
                        </div>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <div class="mt-2">
                                        @foreach ($exception->request['params'] as $key => $value)
                                            <div
                                                class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }} px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4">
